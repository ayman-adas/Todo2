...
const socketIO = socket_io(socketIOServer, {
  cors: {
    origin: "http://localhost:20033", // Update to match your frontend origin
...
class SocketsIo extends Socket {
  ...
  sqlInsertProjectQuery = async function (data) {
    ...
    const result = await new MySql().query(sql, [
      data.ProjectName,
      data.isPrivate,
      data.ProfileID,
      currentDate,
    ...
    socketIO.emit("projects", await this.retrievePublicProjects());
  };
...
//       P.IsPrivate = false
//     AND
//       P.ProjectCreatedTime < NOW() - INTERVAL ${
