...
export default function ListCollaborate( ) {
  ...
  const handleDeleteChange =async (event, ProfileEmail) => {
    try {
      ...
      const response = await axios.delete("http://localhost:2003/project/collabortors/delete", {
        data: {
      ...
      <List component="nav" aria-label="main mailbox folders">
        {Projectollaborate.map((user, index) => (
          <React.Fragment key={index}>
            ...
            >
              <HighlightOffIcon  onClick={(event) => handleDeleteChange(event, user.ProfileEmail)}>
